...
<?php 
	...
	if (count($representantes)>0){
		foreach ($representantes as $key => $value) {
			...
			$links = array(
				array('title'=>'Ver','icon' => 'fa fa-eye','url' => array('personas','representante','ver',$value['representante_id'])),
				array('title'=>'Actualizar','icon' => 'fa fa-pencil','url' => array('personas','representante','actualizar',$value['representante_id'])),
			);
