...
<?php 
	...
	$columns[0] = array(
		array('data' => $this->estudiante->fields()['matricula'], 'width' => '5%'),
	...
	if (count($estudiantes)>0){
		foreach ($estudiantes as $key => $value) {
			$arr = array_intersect_key($value, array_flip(array(
						'matricula','identificacion','nombres','apellidos','telefono_hab','telefono_mov'
					)
				)
			);
			$links = array(
				array('title'=>'Ver','icon' => 'fa fa-eye','url' => array('personas','estudiante','ver',$value['estudiante_id'])),
				array('title'=>'Actualizar','icon' => 'fa fa-pencil','url' => array('personas','estudiante','actualizar',$value['estudiante_id'])),
			);
